...
class gp_theprint_load_trending_posts_b extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_trending_posts_b', // Widget ID.
				esc_html__( '* WGP - Sidebar - Trending Posts B', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-trending-posts-b', // Widget class.
					'description' => esc_html__( 'A widget that displays trending posts with style B.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget($args, $instance) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$trending_posts_b = new WP_Query( array( 
				'showposts' => $posts, 
				'meta_key' => 'gp_theprint_trending_post_add', 
				'meta_value' => '1', 
			) ); 
			?>
				<?php if ( $trending_posts_b->have_posts() ) : ?>
					<ul>
						<?php 
						// Start the loop.
						while( $trending_posts_b->have_posts() ): $trending_posts_b->the_post();
						?>
						
							<li class="gp-theprint-trending-post-b">
							...
							</li><!-- .gp-theprint-trending-post-b -->
...
function register_gp_theprint_load_trending_posts_b() {
    register_widget( 'gp_theprint_load_trending_posts_b' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_trending_posts_b' );
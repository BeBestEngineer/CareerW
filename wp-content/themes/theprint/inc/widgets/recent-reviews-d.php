...
class gp_theprint_load_recent_reviews_d extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_reviews_d', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Reviews D', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-reviews-d', // Widget class.
					'description' => esc_html__( 'A widget that displays recent reviews with style D.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_reviews_d = new WP_Query( array( 
				'showposts' => $posts, 
			...
			?>
				<?php if ( $recent_reviews_d->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						while( $recent_reviews_d->have_posts() ): $recent_reviews_d->the_post();
						?>

							<li class="gp-theprint-recent-review-d">
								<div class="featured-image">
									<?php if ( has_post_thumbnail() ): ?>
										<a href="<?php esc_url( the_permalink() ); ?>">
											<?php the_post_thumbnail( 'gp-theprint-150x110', array( 'alt' => the_title_attribute( 'echo=0' ) ) ); ?>
											
											<?php gp_theprint_entry_format_small(); ?>
											
											<?php gp_theprint_entry_review_badge(); ?>
										</a>
									<?php endif; ?>
								</div><!-- .featured-image -->
							...
							</li><!-- .gp-theprint-recent-review-d -->
...
function register_gp_theprint_load_recent_reviews_d() {
    register_widget( 'gp_theprint_load_recent_reviews_d' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_reviews_d' );
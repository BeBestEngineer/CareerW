...
class gp_theprint_load_recent_posts_a extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_posts_a', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Posts A', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-posts-a', // Widget class.
					'description' => esc_html__( 'A widget that displays recent posts with style A.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_posts_a = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $recent_posts_a->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						while( $recent_posts_a->have_posts() ): $recent_posts_a->the_post(); ?>
						
							<li class="gp-theprint-recent-post-a">
...
function register_gp_theprint_load_recent_posts_a() {
    register_widget( 'gp_theprint_load_recent_posts_a' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_posts_a' );
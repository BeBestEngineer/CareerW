<?php
/**
 * Homepage widget area #2 posts with style F.
 *
 *
...
class gp_theprint_load_homepage_2_posts_f extends WP_Widget {

	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_2_posts_f', // Widget ID.
				esc_html__( '* WGP - Homepage #2 - Posts F', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-2-posts-f', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #2 posts with style F.', 'theprint' ), // Widget description.
				)
		);
		...
		<div class="gp-theprint-columns">
			<div class="gp-theprint-column-1">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$homepage_2_posts_f = new WP_Query( array( 
						'cat' => $categories, 
						'showposts' => $posts, 
						'offset' => $exclude_posts,
					) ); 
					?>
						<?php if ( $homepage_2_posts_f->have_posts() ) : ?>
							<ul>
								<?php 
								// Start the loop.
								$count = 0;
								while( $homepage_2_posts_f->have_posts() ): $homepage_2_posts_f->the_post(); $count++;
								?>

									<?php if ( $count == 1 ): ?>

										<li class="gp-theprint-homepage-2-post-f style-a">
										...
										</li><!-- .gp-theprint-homepage-one-post-f -->

									<?php else: ?>

										<li class="gp-theprint-homepage-2-post-f style-b">
											<header class="entry-header">
												<h5 class="entry-title">
													<a href="<?php esc_url( the_permalink() ); ?>" rel="bookmark"><?php the_title(); ?></a>
												</h5>
										...
										</li><!-- .gp-theprint-homepage-2-post-f -->
			...
			<div class="gp-theprint-column-2">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$second_homepage_2_posts_f = new WP_Query( array(
						'cat' => $second_categories,  
						'showposts' => $posts, 
						'offset' => $exclude_posts,
					) ); 
					?>
						<?php if ( $second_homepage_2_posts_f->have_posts() ) : ?>
							<ul>
								<?php 
								// Start the loop.
								$count = 0;
								while( $second_homepage_2_posts_f->have_posts() ): $second_homepage_2_posts_f->the_post(); $count++;
								?>
			...
			<div class="gp-theprint-column-3">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$third_homepage_2_posts_f = new WP_Query( array(
						'cat' => $third_categories,  
						'showposts' => $posts, 
						'offset' => $exclude_posts,
					) ); 
					?>
						<?php if ( $third_homepage_2_posts_f->have_posts() ) : ?>
							<ul>
								<?php 
								// Start the loop.
								$count = 0;
								while( $third_homepage_2_posts_f->have_posts() ): $third_homepage_2_posts_f->the_post(); $count++;
								?>
...
function register_gp_theprint_load_homepage_2_posts_f() {
    register_widget( 'gp_theprint_load_homepage_2_posts_f' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_2_posts_f' );
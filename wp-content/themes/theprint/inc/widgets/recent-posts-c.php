...
class gp_theprint_load_recent_posts_c extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_posts_c', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Posts C', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-posts-c', // Widget class.
					'description' => esc_html__( 'A widget that displays recent posts with style C.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_posts_c = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $recent_posts_c->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $recent_posts_c->have_posts() ): $recent_posts_c->the_post(); $count++;
						?>

							<?php if ( $count == 1 ): ?>

								<li class="gp-theprint-recent-post-c style-a">
								...
								</li><!-- .gp-theprint-recent-post-c -->

							<?php else: ?>

								<li class="gp-theprint-recent-post-c style-b">
									<div class="featured-image">
										<?php if ( has_post_thumbnail() ): ?>
											<a href="<?php esc_url( the_permalink() ); ?>">
...
function register_gp_theprint_load_recent_posts_c() {
    register_widget( 'gp_theprint_load_recent_posts_c' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_posts_c' );
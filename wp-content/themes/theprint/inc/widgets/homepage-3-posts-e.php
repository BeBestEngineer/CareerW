<?php
/**
 * Homepage widget area #3 posts with style E.
 *
 *
...
class gp_theprint_load_homepage_3_posts_e extends WP_Widget {

	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_3_posts_e', // Widget ID.
				esc_html__( '* WGP - Homepage #3 - Posts E', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-3-posts-e', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #3 posts with style E.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<div class="gp-theprint-columns">
			<div class="gp-theprint-column-1">
				...
				<?php 
				// Start the query.
				$homepage_3_posts_e = new WP_Query( array( 
					'cat' => $categories, 
					'showposts' => $posts, 
					'offset' => $exclude_posts,
				) ); 
				?>
					<?php if ( $homepage_3_posts_e->have_posts() ): ?>
						<ul>
							<?php 
							// Start the loop.
							while( $homepage_3_posts_e->have_posts() ): $homepage_3_posts_e->the_post();
							?>

									<li class="gp-theprint-homepage-3-post-e">
									...
									</li><!-- .gp-theprint-homepage-3-post-e -->
			...
			<div class="gp-theprint-column-2">
				...
				<?php 
				// Start the query.
				$second_homepage_3_posts_e = new WP_Query( array( 
					'cat' => $second_categories, 
					'showposts' => $posts, 
					'offset' => $exclude_posts,
				) ); 
				?>
					<?php if ( $second_homepage_3_posts_e->have_posts() ): ?>
						<ul>
							<?php 
							// Start the loop.
							while( $second_homepage_3_posts_e->have_posts() ): $second_homepage_3_posts_e->the_post();
							?>
			...
			<div class="gp-theprint-column-3">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$third_homepage_3_posts_e = new WP_Query( array( 
					...
					<?php if ( $third_homepage_3_posts_e->have_posts() ): ?>
						<ul>
							<?php 
							// Start the loop.
							while( $third_homepage_3_posts_e->have_posts() ): $third_homepage_3_posts_e->the_post();
							?>
...
function register_gp_theprint_load_homepage_3_posts_e() {
    register_widget( 'gp_theprint_load_homepage_3_posts_e' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_3_posts_e' );
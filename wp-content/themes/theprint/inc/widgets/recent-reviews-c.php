...
class gp_theprint_load_recent_reviews_c extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_reviews_c', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Reviews C', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-reviews-c', // Widget class.
					'description' => esc_html__( 'A widget that displays recent reviews with style C.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_reviews_c = new WP_Query( array( 
				'showposts' => $posts, 
			...
			?>
				<?php if ( $recent_reviews_c->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $recent_reviews_c->have_posts() ): $recent_reviews_c->the_post(); $count++;
						?>

							<?php if ( $count == 1 ): ?>

								<li class="gp-theprint-recent-review-c style-a">
								...
								</li><!-- .gp-theprint-recent-review-c -->

							<?php else: ?>

								<li class="gp-theprint-recent-review-c style-b">
									<div class="featured-image">
										<?php if ( has_post_thumbnail() ): ?>
											<a href="<?php esc_url( the_permalink() ); ?>">
...
function register_gp_theprint_load_recent_reviews_c() {
    register_widget( 'gp_theprint_load_recent_reviews_c' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_reviews_c' );
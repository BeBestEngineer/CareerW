...
class gp_theprint_load_trending_posts_e extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_trending_posts_e', // Widget ID.
				esc_html__( '* WGP - Sidebar - Trending Posts E', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-trending-posts-e', // Widget class.
					'description' => esc_html__( 'A widget that displays trending posts with style E.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$trending_posts_e = new WP_Query( array( 
				'showposts' => $posts, 
				'meta_key' => 'gp_theprint_trending_post_add', 
				'meta_value' => '1', 
			) ); 
			?>
				<?php if ( $trending_posts_e->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $trending_posts_e->have_posts() ): $trending_posts_e->the_post(); $count++;
						?>
...
function register_gp_theprint_load_trending_posts_e() {
    register_widget( 'gp_theprint_load_trending_posts_e' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_trending_posts_e' );
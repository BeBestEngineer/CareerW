<?php
/**
 * Homepage widget area #3 posts with style A.
 *
 *
...
class gp_theprint_load_homepage_3_posts_a extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_3_posts_a', // Widget ID.
				esc_html__( '* WGP - Homepage #3 - Posts A', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-3-posts-a', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #3 posts with style A.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$homepage_3_posts_a = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $homepage_3_posts_a->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						while( $homepage_3_posts_a->have_posts() ): $homepage_3_posts_a->the_post();
						?>

							<li class="gp-theprint-homepage-3-post-a">
							...
							</li><!-- .gp-theprint-homepage-3-post-a -->
...
function register_gp_theprint_load_homepage_3_posts_a() {
    register_widget( 'gp_theprint_load_homepage_3_posts_a' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_3_posts_a' );
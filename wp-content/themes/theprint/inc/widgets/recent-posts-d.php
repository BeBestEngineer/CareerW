...
class gp_theprint_load_recent_posts_d extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_posts_d', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Posts D', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-posts-d', // Widget class.
					'description' => 'A widget that displays recent posts with style D.', 
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_posts_d = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $recent_posts_d->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						while( $recent_posts_d->have_posts() ): $recent_posts_d->the_post();
						?>

							<li class="gp-theprint-recent-post-d">
...
function register_gp_theprint_load_recent_posts_d() {
    register_widget( 'gp_theprint_load_recent_posts_d' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_posts_d' );
<?php
/**
 * Homepage widget area #1 posts with style D.
 *
 *
...
class gp_theprint_load_homepage_1_posts_d extends WP_Widget {

	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_1_posts_d', // Widget ID.
				esc_html__( '* WGP - Homepage #1 - Posts D', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-1-posts-d', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #1 posts with style D.', 'theprint' ), // Widget description.
				)
		);
		...
		<div class="gp-theprint-columns">
			<div class="gp-theprint-column-1">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$homepage_1_posts_d = new WP_Query( array( 
						'cat' => $categories, 
						'showposts' => $posts, 
						'offset' => $exclude_posts,
					) ); 
					?>
						<?php if ( $homepage_1_posts_d->have_posts() ) : ?>
							<ul>
								<?php 
								// Start the loop.
								while( $homepage_1_posts_d->have_posts() ): $homepage_1_posts_d->the_post(); 
								?>

									<li class="gp-theprint-homepage-1-post-d">
									...
									</li><!-- .gp-theprint-homepage-1-post-d -->
			...
			<div class="gp-theprint-column-2">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$second_homepage_1_posts_d = new WP_Query( array(
						'cat' => $second_categories,  
						'showposts' => $posts, 
						'offset' => $exclude_posts,
					) ); 
					?>
						<?php if ( $second_homepage_1_posts_d->have_posts() ) : ?>
							<ul>
								<?php 
								// Start the loop.
								while( $second_homepage_1_posts_d->have_posts() ): $second_homepage_1_posts_d->the_post();
								?>
...
function register_gp_theprint_load_homepage_1_posts_d() {
    register_widget( 'gp_theprint_load_homepage_1_posts_d' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_1_posts_d' );
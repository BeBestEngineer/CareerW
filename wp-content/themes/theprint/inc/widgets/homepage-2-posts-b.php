<?php
/**
 * Homepage widget area #2 posts with style B.
 *
 *
...
class gp_theprint_load_homepage_2_posts_b extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_2_posts_b', // Widget ID.
				esc_html__( '* WGP - Homepage #2 - Posts B', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-2-posts-b', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #2 posts with style B.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$homepage_2_posts_b = new WP_Query( array(  
				'cat' => $categories, 
				'showposts' => $posts,
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $homepage_2_posts_b->have_posts() ): ?>
					<ul>
						<?php
						// Start the loop.
						while( $homepage_2_posts_b->have_posts() ): $homepage_2_posts_b->the_post();
						?>

							<li class="gp-theprint-homepage-2-post-b">
							...
							</li><!-- .gp-theprint-homepage-2-post-b -->
...
function register_gp_theprint_load_homepage_2_posts_b() {
    register_widget( 'gp_theprint_load_homepage_2_posts_b' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_2_posts_b' );
<?php
/**
 * Homepage widget area #3 posts with style C.
 *
 *
...
class gp_theprint_load_homepage_3_posts_c extends WP_Widget {

	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_3_posts_c', // Widget ID.
				esc_html__( '* WGP - Homepage #3 - Posts C', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-3-posts-c', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #3 posts with style C.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<div class="gp-theprint-columns">
			<div class="gp-theprint-column-1">
				...
				<?php 
				// Start the query.
				$homepage_3_posts_c = new WP_Query( array( 
					'cat' => $categories,
					'showposts' => $posts,  
					'offset' => $exclude_posts,
				) ); 
				?>
					<?php if ( $homepage_3_posts_c->have_posts() ): ?>
						<ul>
							<?php 
							// Start the loop.
							$count = 0;
							while( $homepage_3_posts_c->have_posts() ): $homepage_3_posts_c->the_post(); $count++;
							?>

								<?php if ( $count == 1 ): ?>

									<li class="gp-theprint-homepage-3-post-c style-a">
									...
									</li><!-- .gp-theprint-homepage-3-post-c -->

								<?php else: ?>

									<li class="gp-theprint-homepage-3-post-c style-b">
										<div class="featured-image">
											<?php if ( has_post_thumbnail() ): ?>
												<a href="<?php esc_url( the_permalink() ); ?>">
			...
			<div class="gp-theprint-column-2">
				...
				<?php 
				// Start the query.
				$second_homepage_3_posts_c = new WP_Query( array( 
					'cat' => $second_categories, 
					'showposts' => $posts, 
					'offset' => $exclude_posts,
				) ); 
				?>
					<?php if ( $second_homepage_3_posts_c->have_posts() ): ?>
						<ul>
							<?php 
							// Start the loop.
							$count = 0;
							while( $second_homepage_3_posts_c->have_posts() ): $second_homepage_3_posts_c->the_post(); $count++;
							?>
								...
								<?php if ( $count == 1 ): ?>
									...
									</li><!-- .gp-theprint-homepage-2-post-c -->
			...
			<div class="gp-theprint-column-3">
				...
				<?php endif; ?>
					...
					<?php 
					// Start the query.
					$third_homepage_3_posts_c = new WP_Query( array( 
					...
					?>
						...
						<?php if ( $third_homepage_3_posts_c->have_posts() ): ?>
							<ul>
								<?php 
								// Start the loop.
								$count = 0;
								while( $third_homepage_3_posts_c->have_posts() ): $third_homepage_3_posts_c->the_post(); $count++;
								?>
...
function register_gp_theprint_load_homepage_3_posts_c() {
    register_widget( 'gp_theprint_load_homepage_3_posts_c' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_3_posts_c' );
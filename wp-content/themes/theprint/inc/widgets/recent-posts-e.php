...
class gp_theprint_load_recent_posts_e extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_posts_e', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Posts E', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-posts-e', // Widget class.
					'description' => esc_html__( 'A widget that displays recent posts with style E.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_posts_e = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $recent_posts_e->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $recent_posts_e->have_posts() ): $recent_posts_e->the_post(); $count++;
						?>
						
							<li class="gp-theprint-recent-post-e">
...
function register_gp_theprint_load_recent_posts_e() {
    register_widget( 'gp_theprint_load_recent_posts_e' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_posts_e' );
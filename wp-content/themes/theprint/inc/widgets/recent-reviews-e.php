...
class gp_theprint_load_recent_reviews_e extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_reviews_e', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Reviews E', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-reviews-e', // Widget class.
					'description' => esc_html__( 'A widget that displays recent reviews with style E.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_reviews_e = new WP_Query( array( 
				'showposts' => $posts, 
			...
			?>
				<?php if ( $recent_reviews_e->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $recent_reviews_e->have_posts() ): $recent_reviews_e->the_post(); $count++;
						?>
						
							<li class="gp-theprint-recent-review-e">
							...
							</li><!-- .gp-theprint-recent-review-e -->
...
function register_gp_theprint_load_recent_reviews_e() {
    register_widget( 'gp_theprint_load_recent_reviews_e' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_reviews_e' );
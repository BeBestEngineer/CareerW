<?php
/**
 * Homepage widget area #1 posts with style A.
 *
 *
...
class gp_theprint_load_homepage_1_posts_a extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_homepage_1_posts_a', // Widget ID.
				esc_html__( '* WGP - Homepage #1 - Posts A', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-homepage-1-posts-a', // Widget class.
					'description' => esc_html__( 'A widget that displays homepage widget area #1 posts with style A.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$homepage_1_posts_a = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $homepage_1_posts_a->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $homepage_1_posts_a->have_posts() ): $homepage_1_posts_a->the_post(); $count++;
						?>

							<?php if ( $count == 1 || $count == 2 || $count == 6 || $count == 7 ): ?>

								<li class="gp-theprint-homepage-1-post-a style-a">
								...
								</li><!-- .gp-theprint-homepage-1-post-a -->

							<?php else: ?>

								<li class="gp-theprint-homepage-1-post-a style-b">
									<div class="featured-image">
										<?php if ( has_post_thumbnail() ): ?>
											<a href="<?php esc_url( the_permalink() ); ?>">
												<?php the_post_thumbnail( 'gp-theprint-355x220', array( 'alt' => the_title_attribute( 'echo=0' ) ) ); ?>
...
function register_gp_theprint_load_homepage_1_posts_a() {
    register_widget( 'gp_theprint_load_homepage_1_posts_a' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_homepage_1_posts_a' );
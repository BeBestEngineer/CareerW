...
class gp_theprint_load_recent_posts_f extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_recent_posts_f', // Widget ID.
				esc_html__( '* WGP - Sidebar - Recent Posts F', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-recent-posts-f', // Widget class.
					'description' => esc_html__( 'A widget that displays recent posts with style F.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$recent_posts_f = new WP_Query( array( 
				'cat' => $categories, 
				'showposts' => $posts, 
				'offset' => $exclude_posts,
			) ); 
			?>
				<?php if ( $recent_posts_f->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $recent_posts_f->have_posts() ): $recent_posts_f->the_post(); $count++;
						?>
						
							<li class="gp-theprint-recent-post-f">
...
function register_gp_theprint_load_recent_posts_f() {
    register_widget( 'gp_theprint_load_recent_posts_f' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_recent_posts_f' );
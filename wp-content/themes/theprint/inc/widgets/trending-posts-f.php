...
class gp_theprint_load_trending_posts_f extends WP_Widget {
	
	/* Widget setup. */

	public function __construct() {
		parent::__construct(
			'gp_theprint_trending_posts_f', // Widget ID.
				esc_html__( '* WGP - Sidebar - Trending Posts F', 'theprint' ), // Widget name.
				array( 
					'classname' => 'gp-theprint-trending-posts-f', // Widget class.
					'description' => esc_html__( 'A widget that displays trending posts with style F.', 'theprint' ), // Widget description.
				)
		);
	...
	public function widget( $args, $instance ) {
		...
		<?php endif; ?>
			...
			<?php 
			// Start the query.
			$trending_posts_f = new WP_Query( array( 
				'showposts' => $posts, 
				'meta_key' => 'gp_theprint_trending_post_add', 
				'meta_value' => '1', 
			) ); 
			?>
				<?php if ( $trending_posts_f->have_posts() ): ?>
					<ul>
						<?php 
						// Start the loop.
						$count = 0;
						while( $trending_posts_f->have_posts() ): $trending_posts_f->the_post(); $count++;
						?>
						
							<li class="gp-theprint-trending-post-f">
							...
							</li><!-- .gp-theprint-trending-post-f -->
...
function register_gp_theprint_load_trending_posts_f() {
    register_widget( 'gp_theprint_load_trending_posts_f' );
}

add_action( 'widgets_init', 'register_gp_theprint_load_trending_posts_f' );
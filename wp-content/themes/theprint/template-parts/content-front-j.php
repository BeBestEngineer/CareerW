<?php
/**
 * The template used for displaying content with style J.
 *
 * @package WordPress
...
?>
	...
	<?php if ( ($count == 1 || $count == 4 || $count == 7 || $count == 11 || $count == 14 || $count == 17 || $count == 21 || $count == 24 || $count == 27) ) : ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('gp-theprint-front-post-j style-a front-post'); ?>>
	...
	<?php else: ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('gp-theprint-front-post-j style-b front-post'); ?>>
			<div class="featured-image">
				<?php if ( has_post_thumbnail() ): ?>
					<a href="<?php esc_url( the_permalink() ); ?>">
						<?php the_post_thumbnail( 'gp-theprint-360x260', array( 'alt' => the_title_attribute( 'echo=0' ) ) ); ?>

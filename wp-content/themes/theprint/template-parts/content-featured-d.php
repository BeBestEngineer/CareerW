<?php
/**
 * The template used for displaying featured content with style d.
 *
 * @package WordPress
	...
	if( is_home() && !is_paged() ): ?>
		<div class="gp-theprint-featured-posts">
			<div class="gp-theprint-articles">
				<?php 
				// Start the query.
				$featured_posts_d = new WP_Query( array(
					'showposts' => 5, 
					'meta_key' => 'gp_theprint_featured_post_add', 
					'meta_value' => '1',
				) ); 
				?>
					<?php if ( $featured_posts_d->have_posts() ) : ?>
						<?php 
						// Start the loop.
						$count = 0;
						while( $featured_posts_d->have_posts() ): $featured_posts_d->the_post(); $count++;
						?>
						
							<?php if ( $count == 1 ): ?>

								<div class="gp-theprint-featured-post-d featured-post style-a">
									<div class="featured-image">
										...
										<?php if ( has_post_thumbnail() ) : ?>
											<?php the_post_thumbnail( 'gp-theprint-589x562', array( 'alt' => the_title_attribute( 'echo=0' ) ) ); ?>
										<?php endif; ?>
								...
								</div><!-- .gp-theprint-featured-post-d -->

							<?php else: ?>

								<div class="gp-theprint-featured-post-d featured-post style-b">
									<div class="featured-image">
										<?php if ( get_option( 'gp_theprint_featured_posts_shadow_style' ) == 'style-d' ): ?>
											<div class="entry-perfect-shadow"></div><!-- .entry-perfect-shadow -->
										<?php else: ?>
